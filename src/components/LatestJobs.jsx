...
////////////////////// TSQ INFINITE QUERY OPTIMIZATION ////////////////////////////////////////////////
import LatestJobCard from "./LatestJobCard";
import { useSelector } from "react-redux";
import { Link } from "react-router-dom";
import InfiniteScroll from "react-infinite-scroll-component";
...
const LatestJobs = () => {
  const { allJobs } = useSelector((store) => store.job);
  const { loadMoreJobs, hasMore, isFetching } = useGetAllJobs();

  const endMessage = (
    <p className="text-lg font-semibold text-gray-200 pt-5 text-center">
      No More Jobs To Display!
    </p>
  );

  return (
    <div className="max-w-7xl md:mx-auto mx-3 pb-20 overflow-x-hidden mt-12 md:mt-16 m-auto flex flex-col">
      <h1 className="text-2xl md:text-4xl font-bold">
        <span className="text-[#6A38C2]">Latest and Top</span>
        <span className="text-white"> Job Openings</span>
      </h1>
