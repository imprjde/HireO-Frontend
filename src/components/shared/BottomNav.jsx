...
export default function BottomNav() {
  ...
  const fetchNotifications = async () => {
    ...
    console.log("TAN=", resp.da);

    return resp.data.data;
  };

  const { data: notificationCount } = useQuery({
    queryKey: ["notificationCount", authUser?._id],
    queryFn: fetchNotifications,
    enabled: !!authUser?._id, // Only run if user is authenticated
    staleTime: 90000,
    cacheTime: 90000,
    refetchInterval: 90000, // Refetch every 90 seconds
    onSuccess: (data) => {
      if (data > 0) {
        dispatch(setUnseenNotificationCount(data));
      }
    },
  });

  // Optional: If you want to update the Redux store immediately on mount
  useEffect(() => {
    if (notificationCount) {
      dispatch(setUnseenNotificationCount(notificationCount));
    }
